...
public interface Processor<IN, OUT> extends ConfigurationObject {
     ...
     * </li>
     * <li>
     * map it using the input mapper, (if the input mapper does not have a mapping for the property then the
     * unmapped property name is used)
     * </li>
     * <li>
     * Look up the property value in the {@link org.mapfish.print.output.Values} object using the mapped
     * property name
     * </li>
     * <li>
     * set the value on the instance created by this method.  If the value is null an exception will be
     * thrown
     * <em>UNLESS</em> the {@link org.mapfish.print.parser.HasDefaultValue} annotation is on the field for
     * the property.
     * </li>
     * </ol>
     *
     ...
     *         org.mapfish.print.output.Values} object.  The key in the
     *         {@link org.mapfish.print.output.Values}
     *         object is the name of the field or if there is a mapping in the {@link #getOutputMapperBiMap()}
    ...
    /**
     * Get the prefix to apply to each input value.  This provides a simple way to make all output values have
     * unique values.
     * <p>
     * If input prefix is non-null and non-empty (whitespace is removed) then the prefix will be prepended to
     * the normal name of the input value.
     * </p>
     * <p>
     * When a prefix is appended the normal name will be capitalized.  For example: if the normal name is:
     * <em>map</em> and the prefix is <em>page1</em> then the final name will be <em>page1Map</em>.
     * </p>
     * <p>
     * Note: If a mapping is in the {@link #getInputMapperBiMap()} then the prefix will be ignored for that
    ...
    /**
     * Get the prefix to apply to each output value.  This provides a simple way to make all output values
     * have unique values.
     * <p>
     * If output prefix is non-null and non-empty (whitespace is removed) then the prefix will be prepended to
     * the normal name of the output value.
     * </p>
     * <p>
     * When a prefix is appended the normal name will be capitalized.  For example: if the normal name is:
     * <em>map</em> and the prefix is <em>page1</em> then the final name will be <em>page1Map</em>.
     * </p>
     * <p>
     * Note: If a mapping is in the {@link #getOutputMapperBiMap()} then the prefix will be ignored for that
